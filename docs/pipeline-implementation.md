...
5. Deploy the application with a scheduler supporting the every-minute `/api/cron/worker` entry in `vercel.json`. Local development requires an external scheduler or authenticated calls to that route; opening a dashboard no longer runs background provider work. Existing daily watch/briefing/cleanup schedules also require the cron secret.
